...
const transactionResolvers = {
  Query: {
    getTransactions: async (_, { filter: filterVal }, context) => {
      console.log("transaction running", context?.user);
      try {
        ...
        // Initialize amount filter separately
        ...
        filter.userId = context?.user?._id;
    ...
    getBalanceByDate: async () => {
      ...
      for (const transaction of transactions) {
        ...
        const existingEntry = accountData.find(
      ...
      try {
        ...
        transactions.forEach((transaction) => {
          ...
          if (categoryData.length > 0) {
            categoryData[0].transactionCount =
              categoryData[0].transactionCount + 1;
            categoryData[0].totalAmount =
              categoryData[0].totalAmount + transaction.amount;
          } else {
            datas.push({
    ...
    createTransaction: async (_, { input }) => {
      ...
      if (
        ...
        throw new UserInputError(
          "Invalid transaction type. Must be EXPENSE or INCOME."
        );
      }
      if (!categoryId) {
